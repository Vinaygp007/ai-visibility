  ...
  return text.split("\n").map((line, i) => {
    ...
    const renderInline = (t: string) =>
      t
        ...
        .replace(/\*(.+?)\*/g, "<em>$1</em>");
    if (h3) return <h3 key={i} style={{ color: "var(--text)", fontSize: 13, fontWeight: 600, margin: "10px 0 3px" }} dangerouslySetInnerHTML={{ __html: renderInline(h3[1]) }} />;
    if (h2) return <h2 key={i} style={{ color: "var(--text)", fontSize: 14, fontWeight: 700, margin: "12px 0 4px" }} dangerouslySetInnerHTML={{ __html: renderInline(h2[1]) }} />;
    if (h1) return <h1 key={i} style={{ color: "var(--text)", fontSize: 15, fontWeight: 700, margin: "12px 0 5px" }} dangerouslySetInnerHTML={{ __html: renderInline(h1[1]) }} />;
    if (bullet) return (
    ...
    if (numbered) return (
      <div key={i} style={{ display: "flex", gap: 6, margin: "2px 0" }}>
        <span style={{ color: "var(--accent)", flexShrink: 0, minWidth: 16 }}>{numbered[1]}.</span>
...
export default function PromptResponsePanel({ providers, citations = [] }: PromptResponsePanelProps) {
  ...
  const cfg = PROVIDER_CONFIG[activeData?.name ?? ""] ?? { color: "var(--text-muted)", bg: "rgba(var(--overlay-rgb),0.05)", border: "rgba(var(--overlay-rgb),0.12)", icon: "◎" };
  ...
  const citCfg = PROVIDER_CONFIG[activeCitationData?.provider ?? ""] ?? { color: "var(--text-muted)", bg: "rgba(var(--overlay-rgb),0.05)", border: "rgba(var(--overlay-rgb),0.12)", icon: "◎" };
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderColor: "rgba(var(--overlay-rgb),0.06)", background: "rgba(0,0,0,0.18)" }}
      >
        ...
        <div className="flex items-center gap-3">
          ...
          {hasCitations && (
            <div
              ...
              style={{ background: "rgba(var(--overlay-rgb),0.04)", border: "1px solid rgba(var(--overlay-rgb),0.07)" }}
            >
              {(["analysis", "citations"] as const).map((s) => (
                <button
                  ...
                  style={{
                    ...
                    color: section === s ? "var(--accent)" : "var(--text-dim)",
            ...
            <div className="relative" ref={analysisDropdownRef}>
              ...
              >
                ...
                {activeData?.status === "failed" && (
                  <span className="text-[9px] font-mono px-1 rounded" style={{ color: "var(--danger)", background: "rgba(255,90,90,0.12)" }}>✗</span>
                )}
              ...
              {analysisDropdownOpen && (
                <div
                  ...
                  style={{ background: "var(--surface-2)", borderColor: "rgba(var(--overlay-rgb),0.1)", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}
                >
                  {sortedProviders.map((p) => {
                    const pCfg = PROVIDER_CONFIG[p.name] ?? { color: "var(--text-muted)", bg: "rgba(var(--overlay-rgb),0.05)", border: "rgba(var(--overlay-rgb),0.12)", icon: "◎" };
                    const isSelected = activeData?.name === p.name;
                    return (
                      <button
                        key={p.name}
                        onClick={() => { setActiveProvider(p.name); setAnalysisDropdownOpen(false); }}
                        className="w-full flex items-center justify-between px-4 py-2.5 text-[13px] transition-all hover:bg-[rgba(var(--overlay-rgb),0.05)]"
                        style={{ color: isSelected ? pCfg.color : "var(--text)", background: isSelected ? pCfg.bg : "transparent" }}
                      >
                        <div className="flex items-center gap-2">
                          <span style={{ fontSize: 11 }}>{pCfg.icon}</span>
                          <span className="font-medium">{p.name}</span>
                          {p.status === "failed" && (
                            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ color: "var(--danger)", background: "rgba(255,90,90,0.1)" }}>failed</span>
                          )}
                        </div>
                        {p.status === "success" && p.durationMs && (
                          <span className="text-[11px] font-mono" style={{ color: "var(--text-dim)" }}>{p.durationMs}ms</span>
                        )}
                      </button>
                    );
                  })}
              ...
              {citationDropdownOpen && (
                ...
                >
                  {sortedCitations.map((c) => {
                    const pCfg = PROVIDER_CONFIG[c.provider] ?? { color: "var(--text-muted)", bg: "rgba(var(--overlay-rgb),0.05)", border: "rgba(var(--overlay-rgb),0.12)", icon: "◎" };
                    ...
                    return (
                      ...
                      >
                        ...
                        {c.status === "success" && (
                          <span className="text-[11px] font-mono font-semibold" style={{ color: c.count >= 3 ? "var(--success)" : c.count > 0 ? "var(--warning)" : "var(--danger)" }}>
          ...
          {activeData?.status === "failed" ? (
            <div className="p-8 text-center">
              <div className="rounded-xl p-5 inline-block" style={{ background: "rgba(255,90,90,0.05)", border: "1px solid rgba(255,90,90,0.2)" }}>
                <p className="text-[13px] font-medium mb-2" style={{ color: "var(--danger)" }}>✗ Request Failed</p>
                <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>{activeData.error || "No error details available"}</p>
              </div>
            </div>
          ) : activeData ? (
              ...
              <div className="flex-1 p-5 overflow-y-auto" style={{ borderRight: "1px solid rgba(var(--overlay-rgb),0.07)", maxHeight: 560 }}>
                ...
                <div className="mb-4">
                  ...
                  <div
                    ...
                    style={{ color: "var(--text-muted)", background: "rgba(var(--overlay-rgb),0.03)", border: "1px solid rgba(var(--overlay-rgb),0.07)", maxHeight: 120, overflowY: "auto" }}
                  >
                    {activeData.prompt}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Response</p>
                    {activeData.durationMs && (
                      <span className="text-[10px] font-mono" style={{ color: "var(--text-dim)" }}>· {activeData.durationMs}ms</span>
                    )}
                  </div>
                  <button
                ...
                <div className="text-[12.5px] leading-relaxed" style={{ color: "var(--text)" }}>
                  {responseText
                    ...
                    : <span style={{ color: "var(--text-dim)" }}>No response captured</span>}
                </div>
              </div>
              ...
              <div className="flex flex-col" style={{ width: 220, flexShrink: 0, background: "rgba(0,0,0,0.15)" }}>
                <div className="px-4 py-4">
                  <div className="flex items-baseline gap-2 mb-3">
                    ...
                    <span className="text-[11px] font-mono" style={{ color: "var(--text-muted)" }}>{mentions.length}</span>
                  </div>
                  {mentions.length > 0 ? (
                    <div className="flex flex-col gap-2">
                      {mentions.map((m, i) => (
                        <div key={m} className="flex items-center gap-2.5">
                          <span className="text-[11px] font-mono w-4 flex-shrink-0" style={{ color: "var(--text-dim)" }}>{i + 1}</span>
                          <div
                          ...
                          <span className="text-[12px] font-medium text-[var(--text)] truncate">{m}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-[11px]" style={{ color: "var(--text-dim)" }}>No entities extracted</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {section === "citations" && hasCitations && (
        <>
          {activeCitationData?.status !== "success" ? (
            <div className="p-8 text-center">
              <div className="rounded-xl p-5 inline-block" style={{ background: "rgba(255,90,90,0.05)", border: "1px solid rgba(255,90,90,0.2)" }}>
                <p className="text-[13px] font-medium mb-2" style={{ color: "var(--danger)" }}>✗ Citation Query Failed</p>
                <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>{activeCitationData?.error || "No details available"}</p>
              </div>
            </div>
          ) : activeCitationData ? (
            <div className="flex" style={{ minHeight: 320 }}>
              ...
              <div className="flex-1 p-5 overflow-y-auto" style={{ borderRight: "1px solid rgba(var(--overlay-rgb),0.07)", maxHeight: 560 }}>
                ...
                <div className="mb-4">
                  ...
                  <div className="text-[12px] leading-relaxed px-4 py-3 rounded-xl" style={{ color: "var(--text)", background: "rgba(var(--overlay-rgb),0.03)", border: "1px solid rgba(var(--overlay-rgb),0.07)" }}>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>Response</p>
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1 rounded-full" style={{ background: "rgba(var(--overlay-rgb),0.08)" }}>
                        <div
                          className="h-full rounded-full transition-all duration-700"
                          style={{
                            width: `${Math.min(100, (activeCitationData.count / 10) * 100)}%`,
                            background: activeCitationData.count >= 3 ? "var(--success)" : activeCitationData.count > 0 ? "var(--warning)" : "var(--danger)",
                          }}
                        />
                      </div>
                      <span className="text-[11px] font-mono font-semibold" style={{
                        color: activeCitationData.count === 0 ? "var(--danger)" : activeCitationData.count < 3 ? "var(--warning)" : "var(--success)"
                      }}>
                ...
                <div className="text-[12.5px] leading-relaxed" style={{ color: "var(--text)" }}>
                  {activeCitationData.rawAnswer
                    ...
                    : <span style={{ color: "var(--text-dim)" }}>No answer captured</span>}
                </div>
              </div>
              ...
              <div className="flex flex-col" style={{ width: 240, flexShrink: 0, background: "rgba(0,0,0,0.15)" }}>
                ...
                <div className="px-4 py-4 border-b" style={{ borderColor: "rgba(var(--overlay-rgb),0.07)" }}>
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-[12px] font-semibold text-[var(--text)]">Mentions</span>
                    <span className="text-[11px] font-mono" style={{ color: "var(--text-muted)" }}>{dedupCitMentions.length}</span>
                  </div>
                ...
                <div className="px-4 py-4 flex-1 overflow-y-auto" style={{ maxHeight: 280 }}>
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-[12px] font-semibold text-[var(--text)]">Cited pages</span>
                    <span className="text-[11px] font-mono" style={{ color: "var(--text-muted)" }}>{citedUrls.length}</span>
                  </div>
                  {citedUrls.length > 0 ? (
                      ...
                      {citedUrls.map((url, i) => {
                        ...
                        return (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2 group">
                            <span className="text-[11px] font-mono w-4 flex-shrink-0 mt-0.5" style={{ color: "var(--text-dim)" }}>{i + 1}</span>
                            <div
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="text-[11px] font-semibold group-hover:underline leading-tight block truncate" style={{ color: "var(--text)" }}>{domain}</span>
                              {path && <span className="text-[10px] font-mono leading-tight block truncate" style={{ color: "var(--text-dim)" }}>{path}</span>}
                            </div>
                          </a>
                        );
                      })}
                    ...
                    <p className="text-[11px]" style={{ color: "var(--text-dim)" }}>No source URLs captured</p>
                  )}
                </div>
              </div>
            </div>
