...
=======
  ...
  const cfg = PROVIDER_CONFIG[activeData?.name ?? ""] ?? { color: "var(--c-muted)", bg: "var(--c-surface2)", border: "var(--c-border-strong)", icon: "◎" };
  const responseText = activeData?.status === "success" ? extractResponseText(activeData.rawResponse ?? "") : null;
  const mentions = responseText ? extractMentions(responseText) : [];

  const activeCitationData = citations.find((c) => c.provider === activeCitation) ?? citations[0];
  const citCfg = PROVIDER_CONFIG[activeCitationData?.provider ?? ""] ?? { color: "var(--c-muted)", bg: "var(--c-surface2)", border: "var(--c-border-strong)", icon: "◎" };
              ...
              {(["analysis", "citations"] as const).map((s) => (
                <button
                  ...
                  style={{
                    ...
                    color: section === s ? "var(--c-accent)" : "var(--c-muted)",
            ...
            <div className="relative" ref={analysisDropdownRef}>
              ...
              >
                ...
                {activeData?.status === "failed" && (
                  <span className="text-[9px] font-mono px-1 rounded" style={{ color: "var(--c-error)", background: "rgba(255,90,90,0.12)" }}>✗</span>
                )}
...
=======
                  {providers.map((p) => {
                    const pCfg = PROVIDER_CONFIG[p.name] ?? { color: "var(--c-muted)", bg: "var(--c-surface2)", border: "var(--c-border-strong)", icon: "◎" };
>>>>>>> 8901276d822b8664fbde2b0d52dea00f2dc90a05
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {p.status === "failed" && (
                            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ color: "var(--c-error)", background: "rgba(255,90,90,0.1)" }}>failed</span>
                          )}
                        </div>
                        {p.status === "success" && p.durationMs && (
                          <span className="text-[11px] font-mono" style={{ color: "var(--c-muted)" }}>{p.durationMs}ms</span>
                        )}
                      </button>
                    );
                  })}
...
=======
                  {citations.map((c) => {
                    const pCfg = PROVIDER_CONFIG[c.provider] ?? { color: "var(--c-muted)", bg: "var(--c-surface2)", border: "var(--c-border-strong)", icon: "◎" };
>>>>>>> 8901276d822b8664fbde2b0d52dea00f2dc90a05
                    ...
                    return (
                      ...
                      >
                        ...
                        {c.status === "success" && (
                          <span className="text-[11px] font-mono font-semibold" style={{ color: c.count >= 3 ? "var(--c-success)" : c.count > 0 ? "var(--c-warning)" : "var(--c-error)" }}>
          ...
          {activeData?.status === "failed" ? (
            <div className="p-8 text-center">
              <div className="rounded-xl p-5 inline-block" style={{ background: "rgba(255,90,90,0.05)", border: "1px solid rgba(255,90,90,0.2)" }}>
                ...
                <p className="text-[12px]" style={{ color: "var(--c-muted)" }}>{activeData.error || "No error details available"}</p>
              </div>
            </div>
          ) : activeData ? (
            <div className="flex" style={{ minHeight: 320 }}>
              ...
              <div className="flex-1 p-5 overflow-y-auto" style={{ borderRight: "1px solid var(--c-border)", maxHeight: 560 }}>
                {/* Prompt */}
                <div className="mb-4">
                  <p className="text-[10px] font-mono uppercase tracking-wider mb-2" style={{ color: "var(--c-muted)" }}>
                    Prompt sent to {activeData.name}
                  </p>
                  <div
                    className="text-[12px] leading-relaxed px-4 py-3 rounded-xl"
                    style={{ color: "var(--c-muted)", background: "var(--c-surface2)", border: "1px solid var(--c-border)", maxHeight: 120, overflowY: "auto" }}
                  >
                    {activeData.prompt}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "var(--c-muted)" }}>Response</p>
                    {activeData.durationMs && (
                      <span className="text-[10px] font-mono" style={{ color: "var(--c-muted)" }}>· {activeData.durationMs}ms</span>
                    )}
                  </div>
                  <button
                ...
                <div className="text-[12.5px] leading-relaxed" style={{ color: "var(--c-text-sub)" }}>
                  {responseText
                    ...
                    : <span style={{ color: "var(--c-muted)" }}>No response captured</span>}
                </div>
              </div>

              {/* Right: Mentions */}
              <div className="flex flex-col" style={{ width: 220, flexShrink: 0, background: "var(--c-surface2)" }}>
      ...
      {section === "citations" && hasCitations && (
        <>
          {activeCitationData?.status !== "success" ? (
            <div className="p-8 text-center">
              <div className="rounded-xl p-5 inline-block" style={{ background: "rgba(255,90,90,0.05)", border: "1px solid rgba(255,90,90,0.2)" }}>
                <p className="text-[13px] font-medium mb-2" style={{ color: "var(--c-error)" }}>✗ Citation Query Failed</p>
                <p className="text-[12px]" style={{ color: "var(--c-muted)" }}>{activeCitationData?.error || "No details available"}</p>
              </div>
            </div>
          ) : activeCitationData ? (
            <div className="flex" style={{ minHeight: 320 }}>
              ...
              <div className="flex-1 p-5 overflow-y-auto" style={{ borderRight: "1px solid var(--c-border)", maxHeight: 560 }}>
                {/* Query */}
                <div className="mb-4">
                  <p className="text-[10px] font-mono uppercase tracking-wider mb-2" style={{ color: "var(--c-muted)" }}>Query</p>
                  <div className="text-[12px] leading-relaxed px-4 py-3 rounded-xl" style={{ color: "var(--c-text-sub)", background: "var(--c-surface2)", border: "1px solid var(--c-border)" }}>
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "var(--c-muted)" }}>Response</p>
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1 rounded-full" style={{ background: "var(--c-border-strong)" }}>
                        <div
                          className="h-full rounded-full transition-all duration-700"
                          style={{
                            width: `${Math.min(100, (activeCitationData.count / 10) * 100)}%`,
                            background: activeCitationData.count >= 3 ? "var(--c-success)" : activeCitationData.count > 0 ? "var(--c-warning)" : "var(--c-error)",
                          }}
                        />
                      </div>
                      <span className="text-[11px] font-mono font-semibold" style={{
                        color: activeCitationData.count === 0 ? "var(--c-error)" : activeCitationData.count < 3 ? "var(--c-warning)" : "var(--c-success)"
                      }}>
              ...
              <div className="flex flex-col" style={{ width: 240, flexShrink: 0, background: "var(--c-surface2)" }}>
                ...
                <div className="px-4 py-4 border-b" style={{ borderColor: "var(--c-border)" }}>
                  <div className="flex items-baseline gap-2 mb-3">
                    <span className="text-[12px] font-semibold" style={{ color: "var(--c-text)" }}>Mentions</span>
                    <span className="text-[11px] font-mono" style={{ color: "var(--c-muted)" }}>{dedupCitMentions.length}</span>
                  </div>
